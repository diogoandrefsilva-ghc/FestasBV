...
- **O 🖨 segue o sub-separador onde se está** (`escopo`): no **🛒 Carrinho** a folha é só o carrinho de quem carrega (`shopMineOwn`, o meu e não o do cônjuge) — é essa a lista que aquela pessoa vai fazer, e aí não se repete o `🛒 quem trata` linha a linha, que seria sempre ela. Nos outros sub-separadores, e em 📊 Relatórios (que se abre fora deste ecrã), é a lista toda.
