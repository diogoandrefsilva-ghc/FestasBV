...
## Bebida da refeição (🍻)
Uma despesa alocada a uma refeição era sempre **comida** — dividida só por quem come. As bebidas compradas para uma noite (e bebidas depois do jantar, por quem jantou **e** por quem só veio ao copo) não cabiam aí: alocadas à refeição deixavam de fora quem só bebe; deixadas na bolsa indireta eram pagas por toda a gente do ano, incluindo quem não lá esteve.

A refeição passa a ter **dois baldes diretos**: `Fdir` (comida) ÷ `Ncomem`, e `Fbeb` (bebida) ÷ `Ncomem + Nbebe`. O custo total não muda — muda quem o paga. O `Lbebe` de quem só bebe passa a ser `indiretoPP + bebidaPP`.
- **A marca é da despesa, não do tipo.** `tipo` continua a ser Almoço/Jantar e a data-valor continua a identificar a refeição — é o que evita a ambiguidade quando há almoço e jantar no mesmo dia. A coluna `despesas.bebida` é só o interruptor. **Não tentar fazer isto com um tipo novo** ("Convívio", "Bebidas do dia"): o tipo não diz a que refeição pertence.
- **No stock não há coluna** — a marca vive no JSON das alocações (`{tipo, data, qtd, bebida:true}`) e o destino serializa-se com um 3.º segmento: `'Jantar|2026-08-15|b'` (`DEST_BEB`). `destinoAloc`/`alocToDestino` são os dois únicos pontos de serialização; quase tudo o resto passa por eles.
- **Onde se perde com facilidade:** todo o sítio que reconstrói alocações (`stkTrimAlocs`, `resolveUmbrella`, o merge do `alocarStockPedido`, `loteAlocField`, `loteAddToGroup`) tem de copiar o flag — e comida e bebida da mesma refeição **não se juntam na mesma linha**, senão a marca desaparece ao distribuir pelos lotes.
- **Anos pré-2026 ficam intactos**: sem modelo "só bebe" (`indiretoComBebe`), a bebida marcada comporta-se como comida e não entra no `Lbebe`.
- A bolsa indireta não é afetada: estas despesas já saíam dela pelo `allocDireta`, e `fracBebidas`/`fracCerveja` filtram por tipo (`Bebidas`/`Cerveja`), que estas não são.
- Marca-se em três sítios: destino do lote (🧺 Stock), destino do artigo (compra/provisória) e botão comida/bebida no cash-flow avulso (só com refeição escolhida).
- Migração: `db/bebida_refeicao.sql`. Tolerante: sem ela, `DESP_BEBIDA_COL=false`, o botão do cash-flow fica escondido e a coluna nunca se grava — **a metade do stock funciona à mesma**, que essa não precisa de coluna.
...
- **Provisória não é compra.** `data_desp` fica a `null` e os artigos **não geram lotes de stock**: dar entrada em stock do que ainda não se comprou fazia a lista dar pedidos por cobertos e mandava quem vai às compras de mãos a abanar. Ficam como **despesas** — uma por artigo (mesmo `compra_id`), nome e quantidade nas observações (`Artigo (qtd)`), preço no valor. É isso que a faz **reabrir itemizada** (`openCompra` reconstrói os lotes de cada despesa) e dá a cada artigo um € seu no custo da refeição. As antigas traziam tudo numa despesa só, separado por vírgulas: `provArtigos` lê as duas e o que não se pode separar fica numa linha só.
- **Destino por artigo, como numa compra.** Cada artigo tem o seu (refeição, tipo, ou 🍻 bebida da refeição) e divide-se por vários com ⇄ — o valor reparte-se pelas quantidades do split, com o resto na última parcela para as parcelas somarem sempre o total escrito. O destino aterra no **tipo + data-valor (+ `bebida`) da linha de despesa do artigo**, não em alocações — não há lotes. Sem destino, Gerais (a bolsa comum). Houve um tempo em que era um destino só para a despesa toda (`compraProvDestHtml`, removido): não dava para uma provisória tocar duas refeições nem para marcar bebida. Reabrir devolve o destino de **cada linha**, por isso as provisórias antigas (todas as linhas com o mesmo) abrem na mesma certas.
...
- Ao editar uma provisória já gravada os tabuladores 💶/∑ desaparecem e ela abre **sempre por artigo** (`det` forçado): as linhas de "repartição do valor" não existem numa provisória — o valor está nos artigos — e alimentar as duas coisas ao mesmo tempo somava tudo a dobrar (por isso `saveCompra` salta o ciclo das `lines` quando é provisória). Quando for paga há dois caminhos: pelo editor de cash-flows mete-se a data e ela passa a movimento efetivo (**sem** virar stock); **aqui dentro**, trocar 📌 → 📅 fecha o ciclo todo — os artigos reabertos são lotes por gravar, já com o destino da provisória, e entram em 🧺 Stock.
- **A fatura (📷) vale aqui.** As linhas lidas que não casam com nada vão para os "extras" — e numa provisória não casa nada, porque não se veio da lista. Esse bloco esteve escondido no modo 📌 e o resultado era ler a fatura e não aparecer nada no ecrã.
