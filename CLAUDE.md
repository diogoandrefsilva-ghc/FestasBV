...
- **👤 Entregar a alguém, num toque** (`qtBtn`/`qtOpen`/`qtSet`, só admin): estava só no `shop-claim` do detalhe de cada pedido, o que dava dois modais para responder uma vez à mesma pergunta. O botão está na linha (cartão e cabeçalho de grupo) e grava para os pedidos **todos** do artigo — a folha di-lo antes de se tocar. Não é um `claim`: o admin escolhe quem, incluindo "ninguém", por isso **não leva a guarda anti-corrida** do ＋🛒 (aqui a decisão dele manda). Trocar de dono limpa o `no_carrinho` — o "já apanhado" é do carrinho de quem tratava e não passa com o artigo. Não aparece no coberto/encomendado/removido, que já estão tratados.
