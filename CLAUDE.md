...
- **Compras** — os pedidos colapsam numa linha só, em secção própria, **fora da ordenação escolhida** (loja/refeição/artigo/categoria). Quantidade = **máximo, nunca soma** (`shopDespQty`) — somar era o erro a corrigir. As contagens do topo contam **grupos**, não pedidos.
...
- **Cobertura** — havendo lote comprado do artigo, todos os pedidos ficam cobertos, sem exigir alocação refeição a refeição. É a única exceção à regra de `shopIsCovered`.
