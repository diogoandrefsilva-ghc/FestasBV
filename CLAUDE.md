...
- **Categoria grande parte-se em pedaços com cabeçalho próprio** (`… (cont.)`, `MAX_BLOCO`), em vez de se deixar cortar sozinha entre colunas: cortada, metade dos artigos ficava no topo da coluna seguinte sem se saber de que categoria eram (numa folha de compras, é mandar a pessoa ao corredor errado); inteira, um bloco mais alto do que a coluna empurrava a folha para a página seguinte.
...
- **Cabe numa folha**: 2 colunas, 3 quando a lista cresce, corpo mais pequeno quando é mesmo grande (`peso` = linhas + cabeçalhos); lista curta (um carrinho, tipicamente) fica **numa coluna à largura toda**. Testado até ~150 artigos numa página A4.
- **As colunas são feitas à mão** (uma `.sl-col` por coluna, blocos repartidos no JS por peso), **não com `column-count`**: o WebKit do iPhone ignora o multicol ao paginar para impressão e a folha saía numa coluna só esticada por duas páginas — o contrário do que se quer. A repartição é sequencial (enche uma coluna até ao seu quinhão e passa à seguinte), para a folha se ler de cima abaixo e a ordem não mudar no telemóvel.
- **No ecrã estreito é UMA coluna à largura toda, com o corpo a tamanho normal** (`@media screen and (max-width:640px)`): em duas colunas de 180px cada linha partia-se em três e a pré-visualização ficava um emaranhado. As colunas e o corpo pequeno são para o **papel**, onde há 800px de largura.
